...
#===============
#This document is an introductory tutorial to using 
#regular expressions in Python with the re module. 
#It provides a gentler introduction than the 
...
#(You can enable a case-insensitive mode that 
#would let this RE match Test or TEST as well;
#more about this later.)

#There are exceptions to this rule; some characters are special
#metacharacters, and dont match themselves. 
#Instead, they signal that some out-of-the-ordinary 
#thing should be matched, or they affect other 
#portions of the RE by repeating them or changing 
#their meaning. Much of this document is devoted 
#to discussing various metacharacters and what they do.

#Heres a complete list of the metacharacters; 
#their meanings will be discussed in the rest of this HOWTO.

r'(. ^ $ * + ? { } [ ] \ | ( ) )'
#The first metacharacters we'll look at are [ and ].
#They're used for specifying a character class,
#which is a set of characters that you wish to match.
#Characters can be listed individually,
#or a range of characters can be indicated by 
...
#If you wanted to match only lowercase letters, 
#your RE would be [a-z].

#Metacharacters are not active inside classes.
#For example, 
'(	[akm$]	)'
#will match any of the characters 
r"('a', 'k', 'm', or '$'; '$')" #is usually a metacharacter, 
#but inside a character class its stripped of its special nature.

#You can match the characters not listed within
# the class by complementing the set. This is indicated
#by including a '^' as the first character of the class; 
'^'
...
#\. As in Python string literals, the backslash can be followed
# by various characters to signal various special sequences.
#It's also used to escape all the metacharacters so you can
# still match them in patterns; 
#for example, if you need to match a [ or \, 
...
#For a complete list of sequences and expanded class definitions
#for Unicode string patterns, see the last part of 
#Regular Expression Syntax in the Standard Library reference.
...
'(\S)'
#Matches any non-whitespace character; this is equivalent
#to the class [^ \t\n\r\f\v].
...
#For example, 
'([\s,.]'
#is a character class that will match any whitespace character
#, or ',' or '.'.
...
#Repeating Things
#Being able to match varying sets of characters is 
#the first thing regular expressions can do that isn’t
#already possible with the methods available on strings.
#However, if that was the only additional capability 
#of regexes, they wouldn’t be much of an advance. 
#Another capability is that you can specify that portions
#of the RE must be repeated a certain number of times.

#The first metacharacter for repeating things that we’ll look at is 
*. * 
#doesn’t match the literal character 
'*'
#; instead, it specifies that the previous character 
#can be matched zero or more times, instead of exactly once.

#For example,
ca*t 
#will match 'ct' (0 'a' characters), 'cat' (1 'a'),
#'caaat' (3 'a' characters), and so forth.

#Repetitions such as 
*
#are greedy; when repeating a RE, the matching 
#engine will try to repeat it as many times as possible.
#If later portions of the pattern don’t match, 
#the matching engine will then back up and try again
#with fewer repetitions.

#A step-by-step example will make this more obvious. 
#Let’s consider the expression 
a[bcd]*b. 
#This matches the letter 'a', zero or more letters 
#from the class [bcd], and finally ends with a 'b'. 
...
re.compile('ab*')
re.compile() 
#also accepts an optional flags argument, used to
# enable various special features and syntax variations.
#We’ll go over the available settings later, 
#but for now a single example will do:


p = re.compile('ab*', re.IGNORECASE)
#The RE is passed to re.compile() as a string. 
#REs are handled as strings because regular expressions 
#aren’t part of the core Python language, and no special
#syntax was created for expressing them. 
#(There are applications that don’t need REs at all,
#so there’s no need to bloat the language specification
#by including them.) Instead, the re module is simply a
#C extension module included with Python, just like the 
#socket or zlib modules.

#Putting REs in strings keeps the Python language simpler,
#but has one disadvantage which is the topic of the next section.
...
#This conflicts with Python’s usage of the same character
#for the same purpose in string literals.

#Let’s say you want to write a RE that matches the string 
#\section, which might be found in a LaTeX file. 
#To figure out what to write in the program code, 
#start with the desired string to be matched. 
#Next, you must escape any backslashes and other metacharacters
#by preceding them with a backslash, resulting in the string \\section. The resulting string that must be passed to re.compile() must be \\section. However, to express this as a Python string literal, both backslashes must be escaped again.
...
#as the RE string, because the regular expression must be \\, and each backslash must be expressed as \\ inside a regular Python string literal. In REs that feature backslashes repeatedly, this leads to lots of repeated backslashes and makes the resulting strings difficult to understand.

#The solution is to use Python’s raw string notation 
#for regular expressions; backslashes are not handled 
#in any special way in a string literal prefixed with
#'r', so r"\n" is a two-character string containing '\' 
#and 'n', while "\n" is a one-character string containing a newline.
#Regular expressions will often be written in Python code using this
#raw string notation.

#In addition, special escape sequences that are valid 
#in regular expressions, but not valid as Python string literals,
#now result in a DeprecationWarning and will eventually become a
#SyntaxError, which means the sequences will be invalid if 
#raw string notation or escaping the backslashes isn’t used.
...
#what do you do with it? Pattern objects have several methods
#and attributes. Only the most significant ones will be 
#covered here; consult the re docs for a complete listing.
...
#You can learn about this by interactively experimenting with 
#the re module. If you have tkinter available, you may also want 
#to look at Tools/demo/redemo.py, a demonstration program included
...
#useful when trying to debug a complicated RE.
...
#An empty string shouldn’t match at all, since + means ‘one or more repetitions’. match() should return None in this case, which will cause the interpreter to print no output. You can explicitly print the result of match() to make this clear.
