...
def _attributes_to_string(obj, atrr_dict):
    for k, v in atrr_dict.iteritems():
        if v == True or v == None:
            ...
            v = str(v)
            obj.attributes_string += "%s(%s) " % (k.upper(), v)
...
def findDialog(name):
    """
    Finds dialog by name. Dialog has show(), hide() and close() methods
    close() will destroy the dialog
    """
    uival = "%s_ui.val" % name
    ...
    for dlg in hou.ui.dialogs():
        try:
            ...
            dlg.hide = types.MethodType(hide, dlg)
            dlg.close = types.MethodType(close, dlg)
            return dlg
    return None
...
class HIconButton(HBaseGadget):
    ...
    def __repr__(self):
        _attributes_to_string(self, self.attributes)
        _s = "ACTION_ICONBUTTON \"{icon}\" VALUE({value}) SENDS(1) ".format(
